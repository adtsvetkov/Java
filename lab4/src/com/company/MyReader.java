...
public class MyReader implements MyReaderInterface {
                ...
                while (scanner.hasNextLine() && i < filesize) {
                    ...
                    switch (i) {
                        case 0:
                            ...
                            break;
                        case 1:
                            this.readsize = Integer.parseInt(line);
    ...
    @Override
    public void run() {
        byte[] help_buffer = new byte[readsize];
        FileInputStream fileInputStream = null;
        try {
            fileInputStream = new FileInputStream(filename);
            while (fileInputStream.read(help_buffer) > 0) {
                if (consumers.size() <= threads.size()) {
                    try {
                        threads.poll().join();
                    } catch (InterruptedException e) {
                        log.log("Something went wrong with threads");
                        return;
                    }
                }
                int flag = 0;
                //у нас идеальный мир, поэтому размер файла идеально делится на количество считываемых байт, а количество байт на количество потоков
                for (Consumer i: consumers) {
                    if (this.status() == Status.OK) {
                        if (i.status() == Status.OK) {
                            //закидываем в буффер данные по частям для разных эксекуторов
                            buffer = new byte[help_buffer.length/consumers.size()];
                            System.arraycopy(help_buffer, flag*((byte[])buffer).length, buffer, 0, ((byte[])buffer).length);
                            flag++;
                            if(buffer!=null) {
                                if (i.loadDataFrom(this) != 0) //загружаем данные в консумеры из ридера
                                {
                                    ...
                                    threads.add(thread);
                                    thread.start(); //запускаем конвейер с данными, которые мы залоаддатафромили
                                    this.st = i.status();
               ...
               // help_buffer = new byte[readsize]; //очищаем буфер на случай, если размер последнего блока считывания меньше того, которым читаем; в случае идеального мира просто закомментировать эту строку
            }
        } catch (IOException e) {
            this.st = Status.READER_ERROR;
        } finally {
