...
//делает все большие буквы строки маленькими

//этому эксекьютору не нужен конфиг

public class FourthExecutor implements Executor {
    ...
    private Object doMyJob()
    {
        ...
        helpstring = helpstring.toLowerCase();
        data = helpstring.getBytes();
        return data;
    }
    public FourthExecutor(Logger logger, String config)
    {
        log = logger;
        st = Status.OK;
    ...
    public FourthExecutor(String config, Logger logger)
    {
        log = logger;
        st = Status.OK;
        m_consumers = new ArrayList<>();
